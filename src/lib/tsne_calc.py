...
# The example can be run by executing: ipython tsne_calc.py -pylab
#
#
...
def tsne(inity,pcaflag,X = Math.array([]), no_dims = 2, initial_dims = 50, perplexity = 30.0):
    ...
    eta = 500
    min_gain = 0.01
    if not os.path.isfile(inity):
        print "Creating new initial config"
        Y = Math.random.randn(n, no_dims)
        Math.save(inity,Y)
        Math.savetxt(initytext,Y)
    else:
        print "Retrieving initial config"
        try:
            Y = Math.loadtxt(initytext)
        except IOError:
            Y = Math.load(inity)
