#script originally made by Phil Bradley, Rhiju Das, Michael Tyka, TJ Brunette, and James Thompson from the Baker Lab. Edits done by Steven Combs and Sam Deluca from the Meiler Lab.
...
try:
    from amino_acids import longer_names,modres
except ImportError:
    from helper.amino_acids import longer_names, modres

def check_and_print_pdb(count, residue_buffer, residue_letter, pdbfile, fastaseq):
  ...
  ## if all three backbone atoms are present withoccupancy proceed to print the residue
    ...
    ## count up residue number
        count = count + 1
        return True, fastaseq, pdbfile

    return False, fastaseq, pdbfile
                ...
                if not resnum == oldresnum:
                    if residue_buffer != []:  ## is there a residue in the buffer ?
                      if not residue_invalid:
                        flag1, fastaseq, pdbfile = check_and_print_pdb(count, residue_buffer, residue_letter, pdbfile, fastaseq)
                        if not flag1:
                          ## if unsuccessful
